...
// 파일 읽어서 다른 파일에 저장하는 함수
void rewriteFile(TCHAR filename[], _bstr_t src, _bstr_t trg) {
	ICallClass* cpi = NULL;//DLL 참조
	CoInitialize(NULL);
	HRESULT hr = CoCreateInstance(CLSID_APITranslate, NULL, CLSCTX_INPROC_SERVER, IID_ICallClass, reinterpret_cast<void**>(&cpi));

	FILE* ofp, *ifp;	// ofp: 읽어온 내용을 그대로 쓰는 파일, ifp: 내용을 읽는 파일
	TCHAR buffer[500];

	_tfopen_s(&ifp, filename, _T("rt"));
	_tfopen_s(&ofp, _T(".\\newfile.txt"), _T("a+t"));
	//while (_fgetts(buffer, 100, ifp) != NULL) {
	//	
	//	// 번역 
	//	LPWSTR p1 = _T("");
	//	p1 = cpi->translate(buffer, src, trg);
	//
	//	_fputts(p1, ofp);
	//	//_fputts(buffer, ofp);
	//}

	LPWSTR str = cpi->readfile(filename);
	LPWSTR p1 = _T(""), result;
	LPWSTR temp = wcstok(str, _T("\n"));
	int i = 0;
	result = _T("");
	while (temp != NULL) {
		p1 = cpi->translate(temp, src, trg);
		_fputts(p1, ofp);
		_fputts(_T("\n"), ofp);
